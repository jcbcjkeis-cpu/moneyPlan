...
export default function CalendarHome({
  ...
  // ★ Supabase Realtime 웹소켓 구독: 누군가 DB에 지출/수입을 등록(INSERT)하면 실시간 팝업 알림 발생!
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col w-full min-h-screen pb-28 select-none font-sans relative overflow-hidden">
      ...
      {/* ★ 상단 실시간 알림 팝업 토스트 (누군가 등록하면 부드럽게 등장) */}
      {toastMessage && (
        ...
        <header className="bg-white/70 backdrop-blur-md px-5 py-3 flex items-center justify-between sticky top-0 z-30 border-b border-white/50 shadow-xs transition-all">
          <div className="flex items-center space-x-1">
            <button 
              type="button" 
              onClick={onPrevMonth} 
              className="w-8 h-8 rounded-full bg-white/80 hover:bg-white text-slate-700 flex items-center justify-center transition active:scale-90 shadow-2xs border border-white/60 cursor-pointer"
              title="이전 달"
            >
          ...
          <div className="flex items-center space-x-2">
              ...
              <button
                ...
                className={`px-3 py-1 rounded-full transition-all duration-200 ${
                  currentUserRole === 'husband' 
                    ? 'bg-white text-blue-600 shadow-xs font-bold border border-blue-100/50' 
              ...
              <button
                ...
                className={`px-3 py-1 rounded-full transition-all duration-200 ${
                  currentUserRole === 'wife' 
                    ? 'bg-white text-rose-600 shadow-xs font-bold border border-rose-100/50' 
                    : 'text-slate-500 hover:text-slate-700'
                }`}
              >
                {nicknames.wife}
            ...
            <button 
              ...
              className="w-8 h-8 rounded-full bg-white/80 hover:bg-white text-slate-700 flex items-center justify-center text-xs transition active:scale-90 shadow-2xs border border-white/60 cursor-pointer" 
              title="엑셀 백업"
            >
              📥
            </button>
            <button 
              ...
              className="w-8 h-8 rounded-full bg-white/80 hover:bg-white text-slate-700 flex items-center justify-center text-sm transition active:scale-90 shadow-2xs border border-white/60 cursor-pointer" 
              title="설정"
            >
              ⚙️
            </button>
          ...
          <div className="bg-white/75 backdrop-blur-md rounded-3xl p-5 border border-white/80 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
            <div className="flex justify-between items-baseline mb-4">
              ...
              <div className="text-right">
                ...
                <span className="text-sm font-bold text-emerald-600 bg-emerald-50/90 px-2.5 py-1 rounded-full border border-emerald-200/60 shadow-2xs">
            ...
            <div className="w-full bg-slate-200/60 h-2 rounded-full overflow-hidden mb-2 p-0.5">
              <div 
                className={`h-full rounded-full transition-all duration-500 shadow-xs ${getGaugeColor(consumptionRate)}`} 
              ...
              {Array.from({ length: daysInMonth }, (_, i) => i + 1).map((day) => {
                ...
                return (
                  <button
                    ...
                    className={`relative flex flex-col items-center justify-start pt-1 rounded-2xl transition-all duration-150 group cursor-pointer ${
                      isSelected ? 'bg-white/90 font-bold shadow-xs border border-white' : 'hover:bg-white/50'
                    }`}
                  >
                    <span 
        ...
        <section className="px-5 pt-2">
          <div className="flex items-center justify-between mb-3 px-1">
            <h3 className="text-xs font-bold text-slate-700 flex items-center space-x-1.5">
              ...
              <span className="bg-white/80 border border-white/60 text-slate-700 text-[10px] px-2 py-0.5 rounded-full font-semibold shadow-2xs">
            ...
            <button 
              type="button" 
              onClick={onOpenModal} 
              className="text-xs text-blue-600 font-bold hover:text-blue-700 flex items-center space-x-0.5 transition active:scale-95 cursor-pointer"
            >
              <span>+ 내역 추가</span>
            </button>
          </div>
          ...
          <div className="space-y-2">
            {selectedDayExpenses.length > 0 ? (
              selectedDayExpenses.map((item) => (
                <div 
                  key={item.id} 
                  className="bg-white/80 backdrop-blur-md p-4 rounded-2xl border border-white/80 shadow-2xs hover:bg-white transition-all flex items-center justify-between"
                >
                  ...
                  <div className="flex items-center space-x-3 shrink-0">
                    ...
                    <button
                      ...
                      className="w-7 h-7 rounded-lg bg-white/60 hover:bg-rose-50 text-slate-400 hover:text-rose-500 flex items-center justify-center transition-colors text-sm active:scale-75 border border-white/80 cursor-pointer"
                      title="내역 삭제"
                    >
                      🗑️
                    </button>
